...
function ground_state(basis_line::Vector{BitVector}, z_c::ComplexF64,ϵ::Float64=0.5)
    H =  20I - 10*bilayer_density_matrix(basis_line,ϵ, z_c;layer_choice=1) - 10*bilayer_density_matrix(basis_line,ϵ, -z_c;layer_choice=2)
    E,V = eigs(H, nev=8, which=:SM)
    return V[:,1]
end

function berry_phase(R::Float64, basis_line::Vector{BitVector},ϵ::Float64=0.5)
...
function main()
    ...
    open("berry_phase/$(fname)_two_pins_ϵ_$(epsilon).txt","w+") do f
        for i in 1:length(R_list)
            write(f,"$(R_list[i])\t$(D_list[i])\t$(γ_list[i])\n")
        end
    end

    open("berry_phase/$(fname)_two_pins_Lz_ϵ_$(epsilon).txt","w+") do f
        for i in 1:length(R_list)
